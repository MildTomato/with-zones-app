...
export default class RepoGit implements AnyRepo, SourceRepo, DestinationRepo {
  ...
  _gitCommand: (...args: $ReadOnlyArray<string>) => ShellCommand = (
    ...args
  ) => {
  ...
  configure: () => void = () => {
    const username = "supabase-github-bot";
  ...
  parseDiffHunk: (hunk: string) => null | { +body: string, +path: string } = (
    hunk
  ) => {
